...
export default function AdminPage() {
    ...
    const fetchSummaryLeaves = React.useCallback(async () => {
        ...
        try {
            ...
            // 내일(또는 다음 근무일) 계산: 금요일이면 월요일, 아니면 내일
            let nextDay = addDays(today, 1);
            if (today.getDay() === 5) {
                nextDay = addDays(today, 3);
            }
            setTomorrowLeaves(filterForDay(nextDay));
        } catch (err) {
                        ...
                        <div className="flex-1 flex flex-col min-h-0">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex flex-col gap-1">
                                    <h3 className="text-xl font-black text-slate-800">
                                        {(selectedDay || new Date()).getDay() === 5 ? '월요일의 휴무자' : '내일의 휴무자'}
                                    </h3>
                                </div>
                                <div className="px-3 py-1.5 bg-slate-100 rounded-xl flex items-center gap-2 border border-slate-200">
                                    ...
                                    <span className="text-[11px] font-bold text-slate-700 leading-none pb-0.5">
                                        {format((selectedDay || new Date()).getDay() === 5 ? addDays((selectedDay || new Date()), 3) : addDays((selectedDay || new Date()), 1), 'MM.dd(EEE)', { locale: ko })}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 scrollbar-hide space-y-3">
                                {(() => {
                                    ...
                                    const nextDayLeaves = getDayLeaves(nextDay);
                                    ...
                                    return nextDayLeaves.length > 0 ? nextDayLeaves.map((l, i) => (
